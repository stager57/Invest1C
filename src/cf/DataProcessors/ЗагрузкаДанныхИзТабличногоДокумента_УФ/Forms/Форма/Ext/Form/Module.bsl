...
// Процедура Заполняет список регистров  //ОСК
//

&НаСервере
Процедура УстановитьСписокРегистров()

	Элементы.ИмяВидаРегистра.СписокВыбора.Очистить();
	...
	Если Объект.РежимЗагрузки = 3 Тогда  
		...
		Для каждого МДРегистрСведений Из Метаданные.РегистрыСведений Цикл
			Если МДРегистрСведений.РежимЗаписи <> МДНезависимый Тогда
				Элементы.ИмяВидаРегистра.СписокВыбора.Добавить("РС_" + МДРегистрСведений.Имя, МДРегистрСведений.Синоним);
			КонецЕсли; 
		КонецЦикла;
		Для Каждого МДРегистрНакоплений из Метаданные.РегистрыНакопления Цикл
		     Элементы.ИмяВидаРегистра.СписокВыбора.Добавить(МДРегистрНакоплений.Имя, МДРегистрНакоплений.Синоним);
		КонецЦикла;	
		
		Массив = Новый Массив();
		Если Метаданные.Документы.Найти("КорректировкаРегистров") <> Неопределено Тогда
			Массив.Добавить(Тип("ДокументСсылка.КорректировкаРегистров"));
		КонецЕсли;
		Если Метаданные.Документы.Найти("КорректировкаРегистров") <> Неопределено Тогда
			Массив.Добавить(Тип("ДокументСсылка.ПереносДанных"));
		КонецЕсли;
				
        ТолькоКорректировки = Новый ОписаниеТипов(Массив);	
		
	    Элементы.СсылкаИсточникаДокумента.ОграничениеТипа = ТолькоКорректировки;	
...
КонецПроцедуры // УстановитьСписокРегистров()
...
Функция ПолучитьМетаданныеИсточника()
	...
	ИначеЕсли Объект.РежимЗагрузки = 3 Тогда
		Если Не ПустаяСтрока(Объект.ИмяВидаРегистра) Тогда
			Если Лев(Объект.ИмяВидаРегистра,3) = "РС_" Тогда		
			   Возврат Метаданные.РегистрыСведений[Сред(Объект.ИмяВидаРегистра,4)];
			Иначе	
				Возврат Метаданные.РегистрыНакопления[Объект.ИмяВидаРегистра];
			КонецЕсли;	
		КонецЕсли; 
	КонецЕсли; 
...
Функция ПолучитьТекстВопросаИсточника()
	...
	ИначеЕсли Объект.РежимЗагрузки = 3 Тогда
		...
		Если Объект.СсылкаИсточника.Пустая() Тогда
			Ошибка = "Не выбрана ссылка";
		Иначе
			Если Метаданные.РегистрыНакопления.Найти(МетаданныеИсточника.Имя) =  Неопределено Тогда
				 ТекстСВидомРегистра = "сведений: " ;
			Иначе
				 ТекстСВидомРегистра = "накопления: " ;
			КонецЕсли;
			ТекстВопросаИсточника = " записей в регистр " + ТекстСВидомРегистра + """" + МетаданныеИсточника.Представление() + """";
		КонецЕсли;
...
Функция ЗагрузитьДанныеСервер()
	...
	ИначеЕсли  Объект.РежимЗагрузки = 3 Тогда	     //ОСК
		ОбъектИсточника = Объект.СсылкаИсточника.ПолучитьОбъект();
	КонецЕсли;
	...
	ИначеЕсли Объект.РежимЗагрузки = 1 Тогда
		...
		Источник.Очистить();
	ИначеЕсли Объект.РежимЗагрузки = 2 или Объект.РежимЗагрузки = 3 Тогда
		...
		КонецЦикла;
		Если Объект.РежимЗагрузки = 3 Тогда
			Если ОбъектИсточника.ТаблицаРегистров.Найти(МетаданныеИсточника.Имя, "Имя") = Неопределено Тогда
				НоваяСтрока = ОбъектИсточника.ТаблицаРегистров.Добавить();
				НоваяСтрока.Имя =МетаданныеИсточника.Имя;
			КонецЕсли;	
			
			Если Метаданные.РегистрыНакопления.Найти(МетаданныеИсточника.Имя) =  Неопределено Тогда
				ЗагружаемыйОбъектНаборЗаписи = РегистрыСведений[МетаданныеИсточника.Имя].СоздатьНаборЗаписей();
			Иначе
				ЗагружаемыйОбъектНаборЗаписи = РегистрыНакопления[МетаданныеИсточника.Имя].СоздатьНаборЗаписей();
			КонецЕсли;
			
			ЗагружаемыйОбъектНаборЗаписи.Отбор.Регистратор.Установить(ОбъектИсточника.Ссылка);
			ЗагружаемыйОбъектНаборЗаписи.Прочитать();
	...
	Для К = Объект.ПерваяСтрокаДанныхТабличногоДокумента По ТабличныйДокумент.ВысотаТаблицы Цикл
		...
		ИначеЕсли Объект.РежимЗагрузки = 3 Тогда
			...
			Если Не Объект.ЗамещатьСуществующиеЗаписи Тогда
				ОбъектНайден = ЗагружаемыйОбъектНаборЗаписи.Выбран();
			Иначе
				ОбъектНайден = Ложь;
			КонецЕсли;
			
			ЗагружаемыйОбъект = ЗагружаемыйОбъектНаборЗаписи.Добавить();
	...
	ИначеЕсли Объект.РежимЗагрузки = 3 Тогда
		//Добавить попытку
		ЗагружаемыйОбъектНаборЗаписи.Записать(ОбъектНайден);
...
Процедура ЗаполнитьНастройкиКолонок(Настройки)
	...
	Если ТипЗнч(Настройки) = Тип("ТабличныйДокумент") Тогда
		...
		ИначеЕсли ВосстановленныйРежимЗагрузки = 2 или ВосстановленныйРежимЗагрузки = 3 Тогда
	...
	ИначеЕсли Объект.РежимЗагрузки = 3 Тогда
		...
		Если Метаданные.РегистрыНакопления.Найти(МетаданныеИсточника.Имя)= Неопределено Тогда
			ЗаполнитьНастройкиКолонокРегистраСведений(ТЗ);
		Иначе
			ЗаполнитьНастройкиКолонокРегистраНакопления(ТЗ);
		КонецЕсли;
	КонецЕсли;
...
// Заполняет настройки колонок по умолчанию для регистра накопления  //ОСК
//
&НаСервере
Процедура ЗаполнитьНастройкиКолонокРегистраНакопления(ТЗ)
	...
	Если МетаданныеИсточника.ВидРегистра <> Метаданные.СвойстваОбъектов.ВидРегистраНакопления.Обороты Тогда
...
Процедура ОбновитьДанныеТабличногоДокументаСервер()
	...
	МетаданныеИсточника = ПолучитьМетаданныеИсточника();
	Если Объект.РежимЗагрузки =0 или Объект.РежимЗагрузки =2  или Объект.РежимЗагрузки =3 или МетаданныеИсточника = Неопределено Тогда
		Возврат;
	КонецЕсли;
...
Процедура УправлениеВидимостью()
	...
	ИначеЕсли РежимЗагрузки = 2  или РежимЗагрузки = 3 Тогда
		ТекЭлемент = Элементы.ГруппаЗагрузкаВРегистрСведений;
	Иначе
		Возврат; // Неизвестный режим
	КонецЕсли;
	
	Элементы.СсылкаИсточникаДокумента.Видимость = РежимЗагрузки = 3 ;
	Элементы.СсылкаИсточникаДокумента.АвтоОтметкаНезаполненного = РежимЗагрузки = 3 ;
...
Процедура ИмяВидаРегистраОткрытие(Элемент, СтандартнаяОбработка)
	...
	Если РежимЗагрузки = 3 и Лев(Объект.ИмяВидаРегистра, 3) = "РС_"Тогда
		Форма = ПолучитьФорму("РегистрСведений."+Сред(Объект.ИмяВидаРегистра, 4) + ".ФормаСписка"); 
	ИначеЕсли РежимЗагрузки = 2 Тогда
		Форма = ПолучитьФорму("РегистрСведений."+ Объект.ИмяВидаРегистра + ".ФормаСписка"); 
	Иначе
		Форма = ПолучитьФорму("РегистрНакоплений."+Объект.ИмяВидаРегистра);
	КонецЕсли;
